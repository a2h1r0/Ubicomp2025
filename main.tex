...
\title{Homing Sensor: Implementation of PPG sensor mounting position adjuster device}
...
%% article.
\begin{abstract}
 % ウェアラブルデバイスから取得される生体情報のうち、脈波データはPPGセンサを用いて取得される。PPGセンサは波長約550nmの赤外光、赤色光、または緑色光を発するLEDを皮膚に照射し、その反射光の変化をフォトトランジスタを用いて取得する．この反射光の変化量に基づいて脈波を測定する。しかし、この性質から、センサがゆるく身体に装着されている場合や，血管の少ない身体部位にセンサが装着されていると、正常な脈波データを収集できないという問題があります。そこで我々は、センサが異常な脈波データを検出した時に、センサを移動させて装着部位を調整するデバイスを提案します。提案するデバイスは自動的に位置を調整するため、ユーザが何らかの行動をする必要はありません．これにより、睡眠中のようなセンサの装着位置を調整できないような状況であっても，センサを適切な位置に装着することができ、正しいデータを収集することが可能となる。我々は提案するデバイスの有効性を確認するための評価実験を行った。プロトタイプ装置を実装し、PPGセンサーの取り付け位置を手動で移動させた後と、装置を使用して移動させた後のデータを収集した。収集したデータのピーク数とDTW距離を計算し，観察したところ，手動で移動してもデバイスを使って移動しても取得されるデータに差分がなかった．この結果から、デバイスを使用してPPGセンサを移動することによってデータに影響を与えることはないため，デバイスを使ってセンサの装着位置を適切に調整することは有効であると明らかになった。今後は，リアルタイムに脈波データをモニタリングし，異常値が検出された瞬間にセンサの装着位置を調整できるようなモータ制御アルゴリズムを実装していく予定である．
  % Pulse data is acquired using PPG sensor among the biometric information from wearable devices. The PPG sensor irradiates the skin with LEDs that emit infrared light, red light, or green light with a wavelength of around 550 nm and obtains changes in the reflected light using a phototransistor. The pulse data is measured based on the amount of change in this reflected light. However, due to this nature, normal pulse data cannot be collected if the sensor is worn loosely on the body or if the sensor is worn on a body part with few blood vessels. Therefore, we propose a device that adjusts the mounted position when it detects abnormal pulse data. Since the proposed device automatically adjusts the position, the user does not need to take any action. This allows the sensor to be mounted in the proper position and normal data to be collected even in situations where the sensor mounted position cannot be adjusted, such as during sleep. We conducted an evaluation experiment to validate the effectiveness of our proposed device. We implemented a prototype device and collected data after manually moving the PPG sensor mounted position and after moving it using the device. The number of peaks and DTW distance from the collected data were calculated and observed, and there was no difference in the data acquired whether the position was moved manually or with the device. The results showed that using the device to adjust the PPG sensor mounted position is effectiveness, since moving the sensor using the device does not affect the data. In the future, we plan to implement a motor control algorithm that monitors pulse data in real time and adjusts the sensor mounted position the time when abnormal value is detected.
  Pulse data is acquired using a photoplethysmography (PPG) sensor, one of the biometric sensors commonly used in wearable devices. The PPG sensor illuminates the skin with LEDs that emit infrared, red, or green light (typically around 550 nm in wavelength), and measures changes in the reflected light using a phototransistor. Pulse data is then calculated based on variations in this reflected light. However, due to the nature of this measurement method, reliable pulse data cannot be obtained if the sensor is loosely attached or placed on a body part with few blood vessels. To address this issue, we propose a device that automatically adjusts the sensor’s position when abnormal pulse data is detected. This eliminates the need for manual intervention by the user and enables the sensor to maintain optimal placement, even in situations where adjustments are difficult, such as during sleep. To evaluate the effectiveness of the proposed system, we developed a prototype and conducted an experiment in which the PPG sensor's position was altered both manually and automatically using the device. We analyzed the collected data by comparing the number of peaks and the similarity using the Dynamic Time Warping (DTW) distance. The results showed no significant difference between the two methods, indicating that automatic repositioning by the proposed device does not negatively impact data quality. In future work, we plan to implement a motor control algorithm that monitors pulse data in real time and adjusts the sensor’s position automatically when abnormal values are detected.
\end{abstract}
...
\keywords{pulse wave, heart rate, biometric information, PPG sensor, smartwatch}
...
% 本稿の残りの部分では、\ref{sec:related}セクションで関連研究を紹介します。次に、提案手法の詳細を\ref{sec:device}節で説明し、\ref{sec:evaluation}節で評価する。最後に、\ref{sec:limitation}節で本手法の限界について述べる。
...
\label{sec:related}
% This section introduces effects of sensor wearing position, and using pulse data.
% 脈波データは感情や身体活動の推定などに広く使われるため，生体情報の中でも最も重要なものの一つであると我々は考えている。
...
% 状態認識に脈波データを利用するには，正常な脈波データを収集する必要がある．もし異常な脈波データが混入してしまうと，状態認識精度が低下してしまうなどの影響がある可能性がある．
...
% % \subsection{脈波データの利用}
% % 脈波データは感情や身体活動の推定などに広く使われるため，生体情報の中でも最も重要なものの一つであると我々は考えている。
% We consider pulse data to be one of the most important pieces of biometric data because it is widely used to estimate emotions and physical activity.
...
% There is active research on Remote Photoplethysmography (rPPG) \cite{r_ppg}, which measures heart rate without contact by using video. There are methods to improve the accuracy of heart rate acquisition \cite{r_ppg_heart_rate} and to estimate emotion from the obtained pulse data \cite{r_ppg_emotion_recognition_liu, r_ppg_emotion_recognition_li}.
% % 状態認識に脈波データを利用するには，正常な脈波データを収集する必要がある．もし異常な脈波データが混入してしまうと，状態認識精度が低下してしまうなどの影響がある可能性がある．
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=1\linewidth]{figures/waring.eps}
...
% 収集されたデータのうち最初の5秒間はキャリブレーション期間とし、後半の10秒間のデータを分析に使用した。データの質を分析するために、
% 生データからピーク検出を行った。ピーク検出は心拍数の計測に使用される。
% 事前収集したデータに対し、SciPyのfind_peaksを使用してピーク検出を行った。ピーク同士の最小間隔であるdistanceパラメータは50を設定した。また、我々は正常脈波（指先から）と腕からの脈波の類似度を確認するために，DTWアルゴリズムを使用して波形同士のL1ノルムを計算した。
...
We performed peak detection was performed from the raw data to analyze data quality using \texttt{scipy.signal.find\_peaks}\footnote{\url{https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.find_peaks.html}} of SciPy\footnote{\url{https://scipy.org/}}.% We performed peak detection was performed from the raw data to analyze data quality. Peak detection is used to measure heart rate. \texttt{scipy.signal.find\_peaks}\footnote{\url{https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.find_peaks.html}} of SciPy\footnote{\url{https://scipy.org/}} was used to perform peak detection to the pre-collected data.
...
% PPGセンサは装着位置により取得されるデータが変化するほか、心拍数は常に変化しているため、同じ日に収集したとしても全く同じ波形のデータを取得することはできない。そのため、バンドを使った装着とデバイス上での装着の場合で、腕と指先の脈波の差が変化するかどうかを確認する。
...
\begin{figure}[!t]
    ...
    \begin{minipage}[t]{1\linewidth}
        ...
        \subcaption{The sensor is mounted with a band.}
        \label{fig:manual}
    \end{minipage}
    \begin{minipage}[t]{1\linewidth}
        \centering
        \includegraphics[width=1\linewidth]{figures/auto.eps}
        \subcaption{The sensor is mounted over the device.}
        \label{fig:auto}
    \end{minipage}
    \caption{The raw PPG data when the sensor is mounted and moved.}
\end{figure}

% 検出されたピーク数と、計算されたDTW距離を表\tabref{result}に示す。#は装着位置を表している。腕と指先の脈波のピーク数は2回以内の差であり、デバイスの利用により大きな差が発生することはなかった。また、DTW距離に関しては、Top, Middle, Bottomの順に小さくなる結果となった。デバイスの利用により順位が入れ替わることはなかった。
...
\begin{table}[!t]
    ...
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \subcaption{The sensor is worn with a band.}
    ...
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \subcaption{The sensor is worn over the device.}
        \begin{tabular}{c|cc|c} \hline\hline
            \multirow{2}{*}{\#} & \multicolumn{2}{c|}{Peaks} & \multirow{2}{*}{DTW} \\ \cline{2-3}
             & Arm & Finger & \\ \hline
            1 & 12 & 13 & 174512 \\
...
% 評価実験の結果、デバイスを使用してPPGセンサの装着位置を調整しても、正常なデータを取得できることが明らかになった。
% The results of the evaluation experiment showed that normal data can be obtained even when the device is used to adjust the mounting position of the PPG sensor.
% 今後は、PPGデータが正常な波形か異常な波形かを識別するモデルを構築していく。リアルタイムにPPGデータを取得し、異常な波形を検知することが可能となれば、本論文にて実装したプロトタイプデバイスのモータ制御処理に組み込んでいく。
% そして、正常な脈波データを継続的に取得できるかどうかの評価実験を行う予定である。
...
% Then, we plan to conduct evaluation experiments to see if normal pulse data can be continuously acquired.
...
% 本論文では、センサが異常な脈波データを検出した時に、センサを移動させてセンサの装着部位を調整するデバイスを提案しました。我々はプロトタイプデバイスを実装し，デバイス上のPPGセンサから収集されたデータについて，正しさの評価実験を行った．
% 我々は，バンドを使って腕にセンサ装着して手動で装着位置を調整したあとのデータと，デバイスを使ってセンサの装着位置を腕の上で調整したあとのデータを収集した．また，正常な脈波データとして同時刻の指先の脈波データも収集した．我々は腕と指先から収集されたデータのピーク数および，DTW距離を計算した．
% その結果，デバイスを使って位置を調整した場合であっても，正常なデータを取得できていることが明らかになった．今後は、PPGデータが正常な波形か異常な波形かを識別するモデルを構築し，本論文にて実装したプロトタイプデバイスのモータ制御に組み込んでいく。
