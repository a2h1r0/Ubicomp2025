...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/waring.eps}
...
% 本論文では、センサが異常な脈波データを検出した時に、センサを移動させてセンサの装着部位を調整するデバイスを提案しました。我々はプロトタイプデバイスを実装し，デバイス上のPPGセンサから収集されたデータについて，正しさの評価実験を行った．我々は，バンドを使って腕にセンサ装着して手動で装着位置を調整したあとのデータと，デバイスを使ってセンサの装着位置を腕の上で調整したあとのデータを収集した．また，正常な脈波データとして同時刻の指先の脈波データも収集した．我々は腕と指先から収集されたデータのピーク数および，DTW距離を計算した．その結果，デバイスを使って位置を調整した場合であっても，正常なデータを取得できていることが明らかになった．今後は、PPGデータが正常な波形か異常な波形かを識別するモデルを構築し，本論文にて実装したプロトタイプデバイスのモータ制御に組み込んでいく。
